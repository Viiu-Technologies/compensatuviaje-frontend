...
const statusConfig: Record<string, { label: string; color: string; bgColor: string }> = {
  ...
  pending_review: { label: 'En Cabina', color: 'text-amber-700 dark:text-amber-300', bgColor: 'bg-amber-100 dark:bg-amber-500/10' },
  approved: { label: 'Por Activar', color: 'text-blue-700 dark:text-blue-300', bgColor: 'bg-blue-100 dark:bg-blue-500/10' },
  draft: { label: 'Borrador', color: 'text-slate-600 dark:text-slate-300', bgColor: 'bg-slate-100 dark:bg-slate-800' },
};

// Official 4 ESG Verticals (9 project types)
const projectTypeConfig: Record<string, { label: string; color: string; vertical: string }> = {
  // Bosque
  reforestation: { label: 'Reforestación', color: 'bg-emerald-100 dark:bg-emerald-500/10 text-emerald-700 dark:text-emerald-300', vertical: 'Bosque' },
  conservation: { label: 'Conservación', color: 'bg-emerald-100 dark:bg-emerald-500/10 text-emerald-700 dark:text-emerald-300', vertical: 'Bosque' },
  // Agua
  clean_water: { label: 'Agua Limpia', color: 'bg-blue-100 dark:bg-blue-500/10 text-blue-700 dark:text-blue-300', vertical: 'Agua' },
  water_security: { label: 'Seguridad Hídrica', color: 'bg-blue-100 dark:bg-blue-500/10 text-blue-700 dark:text-blue-300', vertical: 'Agua' },
  // Textil
  circular_economy: { label: 'Economía Circular', color: 'bg-purple-100 dark:bg-purple-500/10 text-purple-700 dark:text-purple-300', vertical: 'Textil' },
  waste_management: { label: 'Gestión de Residuos', color: 'bg-purple-100 dark:bg-purple-500/10 text-purple-700 dark:text-purple-300', vertical: 'Textil' },
  // Social
  energy_efficiency: { label: 'Eficiencia Energética', color: 'bg-amber-100 dark:bg-amber-500/10 text-amber-700 dark:text-amber-300', vertical: 'Social' },
  social_housing: { label: 'Vivienda Social', color: 'bg-amber-100 dark:bg-amber-500/10 text-amber-700 dark:text-amber-300', vertical: 'Social' },
  community_development: { label: 'Desarrollo Comunitario', color: 'bg-amber-100 dark:bg-amber-500/10 text-amber-700 dark:text-amber-300', vertical: 'Social' },
  // Legacy
  renewable_energy: { label: 'Energía Renovable', color: 'bg-amber-100 dark:bg-amber-500/10 text-amber-700 dark:text-amber-300', vertical: 'Legacy' },
  biodiversity: { label: 'Biodiversidad', color: 'bg-emerald-100 dark:bg-emerald-500/10 text-emerald-700 dark:text-emerald-300', vertical: 'Legacy' },
};
                    ...
                    <tr key={project.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-700/30 !transition-colors !group">
                      ...
                      <td className="!px-6 !py-5">
                        <div className="!flex !flex-col !gap-1.5">
                          <span className={`!inline-flex !px-2.5 !py-0.5 !rounded-lg !text-[10px] !font-black !uppercase !tracking-wider !w-fit ${typeInfo?.color || 'bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300'}`}>
                      ...
                      {/* Estado */}
                      <td className="!px-6 !py-5">
                        <span className={`!inline-flex !items-center !gap-1.5 !px-3 !py-1 !rounded-full !text-xs !font-bold ${statusConfig[project.status]?.bgColor || 'bg-slate-100 dark:bg-slate-800'} ${statusConfig[project.status]?.color || 'text-slate-600 dark:text-slate-300'}`}>
