...
const AIKybDetailPage: React.FC = () => {
  ...
  if (loading) {
    return <div className="!p-12 !text-center !text-slate-500">Cargando detalles...</div>;
  }
        ...
        <div className="!flex !items-center !gap-4">
          ...
          <div>
            <div className="!flex !items-center !gap-3 !mb-1">
              <h1 className="!text-2xl !font-bold !text-slate-800">Verificación KYB</h1>
              <AdminPendingBadge aiStatus={evaluation.ai_status} adminDecision={evaluation.admin_decision} />
            </div>
            <p className="!text-slate-500">{evaluation.organization_name}</p>
          </div>
        </div>
      ...
      <div className="!grid !grid-cols-1 lg:!grid-cols-3 !gap-6">
          ...
          <div className="!bg-white !rounded-xl !border !shadow-sm !p-6">
            ...
            <dl className="!space-y-4">
              <div>
                ...
                <dd className="!font-medium !text-slate-900">{evaluation.organization_name}</dd>
              </div>
              <div>
                <dt className="!text-sm !text-slate-500">RUT / Tax ID</dt>
                <dd className="!font-medium !text-slate-900">{evaluation.rut_tax_id || 'No proporcionado'}</dd>
              </div>
              {context && (
                <>
                  <div>
                    <dt className="!text-sm !text-slate-500">Email Contacto</dt>
                    <dd className="!text-slate-900">{context.contact_email}</dd>
                  </div>
                  <div>
                    <dt className="!text-sm !text-slate-500">Estado Actual Cuenta</dt>
                    <dd>
                      <span className="!inline-flex !px-2 !py-0.5 !rounded !text-xs !font-medium !bg-slate-100">
                        {context.status}
                      </span>
                    </dd>
                  </div>
                  <div>
                    <dt className="!text-sm !text-slate-500">Proyectos Creados</dt>
                    <dd className="!text-slate-900">{context.total_projects} proyectos</dd>
                  </div>
                </>
              )}
            </dl>
        ...
        <div className="lg:!col-span-2 !space-y-6">
          <div className="!bg-white !rounded-xl !border !shadow-sm !p-6">
            ...
            {evaluation.scores && (
              <div className="!mb-6">
                <p className="!text-sm !text-slate-500 !mb-2">Score General</p>
                <div className="!flex !items-center !gap-4">
                  ...
                  <span className="!font-bold">{evaluation.scores.overall}%</span>
                </div>
              </div>
            )}
            ...
            {evaluation.ai_insights && (
              <div className="!prose !max-w-none">
                <h4 className="!text-md !font-medium !text-slate-900 !mb-2">Insights Legales</h4>
                <p className="!text-slate-700 !whitespace-pre-line">{evaluation.ai_insights.legal_notes}</p>
                
                <h4 className="!text-md !font-medium !text-slate-900 !mt-4 !mb-2">Insights Financieros</h4>
                <p className="!text-slate-700 !whitespace-pre-line">{evaluation.ai_insights.financial_notes}</p>
                
                <h4 className="!text-md !font-medium !text-slate-900 !mt-4 !mb-2">Insights Técnicos</h4>
                <p className="!text-slate-700 !whitespace-pre-line">{evaluation.ai_insights.technical_notes}</p>
              </div>
            )}
          </div>
        </div>
