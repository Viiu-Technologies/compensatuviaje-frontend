      ...
      <div className="!flex !flex-col sm:!flex-row !justify-between !items-start sm:!items-center !gap-4 !bg-white !p-4 !rounded-xl !shadow-sm !border !border-slate-200">
        ...
        <div className="!flex !items-center !gap-2">
          ...
          <select
            ...
            className="!border-slate-300 !rounded-lg !text-sm !py-2 !pl-3 !pr-8 focus:!ring-indigo-500 focus:!border-indigo-500"
          >
          ...
          <div>
            {certLoading ? (
              ...
              <div className="!p-12 !text-center !text-slate-500">
                ...
                <h3 className="!text-lg !font-medium !text-slate-700 !mb-1">No hay certificaciones</h3>
                    ...
                    {certEvaluations.map((evalItem) => (
                      <tr key={evalItem.id} className="hover:!bg-slate-50 !transition-colors">
                        <td className="!py-3 !px-4">
                          <div className="!font-medium !text-slate-900">{evalItem.project.name}</div>
                          <div className="!text-xs !text-slate-500">{evalItem.partner.name}</div>
                        </td>
                        <td className="!py-3 !px-4">
                        ...
                        <td className="!py-3 !px-4 !text-right">
                          <Link 
                            to={`/admin/evaluations/cert/${evalItem.id}`}
        ...
        {activeTab === 'kyb' && (
          <div>
            {kybLoading ? (
              ...
              <div className="!p-12 !text-center !text-slate-500">
                <Bot className="!w-12 !h-12 !mx-auto !mb-4 !text-slate-300" />
                <h3 className="!text-lg !font-medium !text-slate-700 !mb-1">No hay verificaciones KYB</h3>
                <p>No se encontraron evaluaciones para el filtro seleccionado.</p>
              </div>
            ) : (
              <div className="!overflow-x-auto">
                <table className="!w-full !text-left !border-collapse">
                  ...
                  <tbody className="!divide-y !divide-slate-200">
                    {kybEvaluations.map((evalItem) => (
                      <tr key={evalItem.id} className="hover:!bg-slate-50 !transition-colors">
                        <td className="!py-3 !px-4">
                          <div className="!font-medium !text-slate-900">{evalItem.organization_name}</div>
                          <div className="!text-xs !text-slate-500">Partner ID: {evalItem.partner?.id?.slice(0, 8) || 'N/A'}</div>
                        </td>
                        ...
                        <td className="!py-3 !px-4 !text-right">
                          <Link 
                            to={`/admin/evaluations/kyb/${evalItem.id}`}
                            className="!inline-flex !items-center !gap-1 !px-3 !py-1.5 !text-sm !font-medium !text-indigo-600 !bg-indigo-50 hover:!bg-indigo-100 !rounded-lg !transition-colors"
                          >
                            Revisar
                            <ArrowRight className="!w-4 !h-4" />
