...
// Ported from the ez-tree reference demo (test/trees-treejs/ez-tree/src/app/noise.js)
...
export function simplex2d(v: THREE.Vector2): number {
  ...
  const i = new THREE.Vector2(Math.floor(v.x + C.y * (v.x + v.y)), Math.floor(v.y + C.y * (v.x + v.y)));
  ...
  const iw = new THREE.Vector2(
    i.x - Math.floor(i.x * (1.0 / 289.0)) * 289.0,
    i.y - Math.floor(i.y * (1.0 / 289.0)) * 289.0
  );

  let p = new THREE.Vector3(iw.y, iw.y + i1.y, iw.y + 1.0);
  p = permute3(p);
  p = permute3(new THREE.Vector3(p.x + iw.x, p.y + iw.x + i1.x, p.z + iw.x + 1.0));
